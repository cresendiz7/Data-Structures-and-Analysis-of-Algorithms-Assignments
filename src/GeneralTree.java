...
public class GeneralTree<E> {
	...
	private Node<E> insert(E v, Node<E> currentNode)
	{
		if(currentNode == null)
			return (new Node<E>(v));
		int temp = ((String) v).compareTo((String) currentNode.value);
		if(temp < 0)
			currentNode.firstChild = insert(v,currentNode.firstChild);
		else if(temp > 0)
			currentNode.nextSibling = insert(v, currentNode.nextSibling);
		return currentNode;
	}
	...
	public void traverseDirectories(File f, String indent) {
		if (f.isDirectory()) {
			System.out.println(indent + f.getName());
			indent += "   ";
			for (File sub : f.listFiles()) {
				traverseDirectories(sub, indent);
			}
		} else {
			System.out.println(indent + f.getName());
		}
